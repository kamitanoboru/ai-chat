...
//google翻訳のAPIのURL
var gt_url='https://script.google.com/macros/s/AKfycbzjQrjYn7KlkE_IwS2UlerNo9m7OzGw3_AmPmPP2zY0AMm4aoCx/exec?text=__TEXT__&source=ja&target=en';
...
//テキストとソースとターゲットからページのURLを作成して返す
function getPageURL(text=null,source='ja',target='en') {
  return 'https://script.google.com/macros/s/AKfycbzjQrjYn7KlkE_IwS2UlerNo9m7OzGw3_AmPmPP2zY0AMm4aoCx/exec?text='+text+'&source='+source+'&target='+target;
}
  ...
  $.ajax({
    ...
    success: function(doc) {
      ...
      if(type=="user"){
        ...
        myid = 'xuL5VlJO4pVckWZNZt7zEmVncXE3';
      }else{
      ...
      firebase.database().ref().child("messages/" + currentRoomName).push(tmessage);    
...
function trans2(str,type){
  //APIに投げるURLを作る
  URL=getPageURL(str,lang,'ja');
  ...
  $.ajax({
    ...
    success: function(doc) {
      ...
      if(type=="user"){
        ...
        if(num >= 3){
          var char = Math.ceil(4 * Math.random());
...
function trans3(str,type){
  ...
  $.ajax({
    ...
    success: function(doc) {
      ...
      //firebase.database().ref().child("messages/" + currentRoomName).push(tmessage);    
      var divTagai='<div stlye="margin:4rem">日本語訳:<div class="ai_ja" style="display:none">' + str + '</div></div>';
      //divTagai.appendTo(".message-list");
      $(".message__text").filter(":last").after(divTagai);
      $('.ai_ja').filter(":last").css('margin-bottom','20px');      
      $('.ai_ja').filter(":last").fadeIn(10000);
      //$('.ai_ja:last-child').fadeOut(10000);
...
function aichat(str){
  ...
  var apiKey='DZZtqOuRoNtAGkVii3GghbxMvtdmaN2z';
  
  //jQueryのajaxを利用する場合、dataにformdataをセット
  $.ajax({
    url: url,
    type:'POST',
    data : {apikey: apiKey, query: str}
  }).then(function(data){
    console.log(data);
    if(data.status == '0'){
      var aires=data.results[0]['reply'];
    }else{
      aires="ごめんなさい。意味がわかりませんでした。";
    }
    var myid = 'xuL5VlJO4pVckWZNZt7zEmVncXE3';
    //翻訳されたものを投稿する
    var tmessage = {
      uid: myid,
      text: aires,
...
function ai_cg_chat(str,char){
  
  var url='https://app.cotogoto.ai/webapi/noby.json';
  var apiKey='02d773da2a3437eb3bda61db2d79586c';
  ...
  $.ajax({
    url: url,
    type:'GET',
    data : {appkey: apiKey, text: str, study: '1', persona: char}
  }).then(function(data){
    console.log(data);
    ...
    if(data.status >=  400){
      ...
      var aires=data["text"];
      console.log("char:"+char+"text:"+aires)
    }
    var myid = 'xuL5VlJO4pVckWZNZt7zEmVncXE3';
    //翻訳されたものを投稿する
    var tmessage = {
...
function showView(id) {
  ...
  if (id === "chat") {
    console.log("loadChatViewに行きます");
    loadChatView();

  }else{
    console.log("loadChatViewには行きませんでした");    
  }  
}
...
function onLogin() {
  console.log("ログイン完了");
      ...
      // ルーム内のメッセージも削除
    firebase.database().ref("messages/default").remove();
    
    //ロボットの最初の投稿
    // メッセージを投稿する
    var message = {
      uid: 'xuL5VlJO4pVckWZNZt7zEmVncXE3',
      text: '話しかけてくださいね!',
      time: firebase.database.ServerValue.TIMESTAMP,
    };
    firebase.database().ref().child("messages/default").push(message);    
...
function loadChatView() {
  ...
  favoritesRef.on("child_removed", function(favSnapshot) {
    ...
    $('#message-id--'+messageId+' .message__favorite').removeClass("glyphicon-star").addClass("glyphicon-star-empty");
  ...
  favoritesRef.on("child_added", function(favSnapshot) {
    ...
    $('#message-id--'+messageId+' .message__favorite').removeClass("glyphicon-star-empty").addClass("glyphicon-star");
...
function createMessageDiv(messageId, message) {
  ...
  if(dbdata.favorites && dbdata.favorites[messageId]) {
    // TODO: お気に入りONのとき（glyphicon-star）

    divTag.find(".message__favorite").removeClass("glyphicon-star-empty").addClass("glyphicon-star");

    
  } else {
    // TODO: お気に入りOFFのとき（glyphicon-star-empty）

    divTag.find(".message__favorite").removeClass("glyphicon-star").addClass("glyphicon-star-empty");
...
function toggleFavorite(event) {
  ...
  if(dbdata.favorites && dbdata.favorites[messageId]) {
    ...
    firebase.database().ref('favorites/'+currentUID+'/'+messageId+'/').remove();
    // TODO: お気に入りリンクの glyphicon を glyphicon-star-empty に変更する
    $(this).children('.message__favorite').removeClass("glyphicon-star").addClass("glyphicon-star-empty");
    ...
    // TODO: お気に入りリンクの glyphicon を glyphicon-star に変更する
    $(this).children('.message__favorite').removeClass("glyphicon-star-empty").addClass("glyphicon-star");
    console.log("お気に入りに追加した"+messageId);
  }
...
$(document).ready(function() {
  ...
  $(".lang").click(function(){
    console.log("clicked");
    lang=$(this).attr('id');
    var lang_str=$(this).text();
    console.log(lang);
    $("#select-lang").text("言語:"+lang_str);
  });
